...
import { OPTTAB_WEBSOCKET } from 'app/app.constants';
...
export class TrackerService {
  ...
  receiveVertrag(): Subject<any> {
    return this.listenerSubjectVertrag;
  }
  ...
  subscribe(ssoid: any, betriebsId: any): void {
    ...
    this.connectionSubscription = this.connectionSubject.subscribe(() => {
      if (this.stompClient) {
        this.stompSubscription = this.stompClient.subscribe('/notifications/' + betriebsId, (data: Stomp.Message) => {
          this.counter = 0;

          this.listenerSubject.next(data.body);
        });
        this.stompSubscriptionVertrag = this.stompClient.subscribe('/vertrag/' + betriebsId, (data: Stomp.Message) => {
          this.listenerSubjectVertrag.next(data.body);
        });
      }
    });
  }
