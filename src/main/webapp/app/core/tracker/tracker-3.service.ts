...
import { BETTAB_WEBSOCKET, OPTTAB_WEBSOCKET } from 'app/app.constants';

@Injectable({ providedIn: 'root' })
export class TrackerService3 {
  ...
  private listenerSubjectCamunda: Subject<String> = new Subject();
  ...
  receiveCamunda(): Subject<any> {
    return this.listenerSubjectCamunda;
  }
  ...
  subscribe(ssoid: any, betriebsId: any): void {
    ...
    this.connectionSubscription = this.connectionSubject.subscribe(() => {
      if (this.stompClient) {
        this.stompSubscription = this.stompClient.subscribe('/betrieb/' + betriebsId, (data: Stomp.Message) => {
          this.listenerSubject.next(data.body);
        });
        this.stompSubscription = this.stompClient.subscribe('/betrieb/camunda/' + betriebsId, (data: Stomp.Message) => {
          this.listenerSubjectCamunda.next(data.body);
        });
      }
    });
  }
