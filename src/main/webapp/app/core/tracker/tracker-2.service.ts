...
import { BPTAB_WEBSOCKET, OPTTAB_WEBSOCKET } from 'app/app.constants';

@Injectable({ providedIn: 'root' })
export class TrackerService2 {
  ...
  subscribe(ssoid: any, betriebsId: any): void {
    ...
    this.connectionSubscription = this.connectionSubject.subscribe(() => {
      if (this.stompClient) {
        this.stompSubscription = this.stompClient.subscribe('/benutzer/' + ssoid, (data: Stomp.Message) => {
