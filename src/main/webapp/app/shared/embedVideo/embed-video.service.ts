...
export class EmbedVideoService {
  ...
  public embed(url: any, options?: any): any {
    ...
    id = this.detectAgon(url);
    if (id) {
      return this.embed_agon(id, options);
    }

  }

  public embed_agon(id: string, options?: any): string {
    options = this.parseOptions(options);

    return this.sanitize_iframe('<iframe src="'+ id
...
/*    id = this.detectAgon(url);
  ...
  private detectAgon(url: any): string {

    if (url.hostname === 'a-gon.de') {
      return url;
    }
